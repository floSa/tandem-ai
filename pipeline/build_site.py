...
const EXIGE={frontier:'le coût mesuré de chaque run',
             budget:'le coût mesuré de chaque run',
             harness:"le nom du harnais utilisé"};
function dispo(k,b){
  b=b||bsel.value;
  if(k==='frontier'||k==='budget')
    return D.scores.some(s=>s.b===b&&s.c!=null&&s.c>0);
  if(k==='harness'){
    const vus=new Map();
...
function draw(){majBoutons();DRAW[K]();}
